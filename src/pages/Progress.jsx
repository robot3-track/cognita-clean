...
export default function Progress() {
  ...
  const loadData = useCallback(async () => {
    ...
    // Load ALL sessions (no limit) for accurate totals
    const s = await db.entities.StudySession.filter(
      { user_email: me.email },
      "-created_date",
      500
    );
    ...
    if (s.length === 0) {
      const s2 = await db.entities.StudySession.list("-created_date", 500);
      setSessions(s2.filter(x => x.user_email === me.email || x.created_by === me.email));
    } else {
      setSessions(s);
    }
    setLoading(false);
  ...
  // Live subscription to new sessions
  useEffect(() => {
    let unsub;
    db.auth.me().then(me => {
      unsub = db.entities.StudySession.subscribe((event) => {
        if (event.type === "create") {
          const s = event.data;
          if (s.user_email === me.email || s.created_by === me.email) {
            setSessions(prev => [s, ...prev]);
          }
        } else if (event.type === "update") {
          setSessions(prev => prev.map(x => x.id === event.id ? event.data : x));
  ...
  const studyMinutes = sessions.filter(s => s.session_type !== "browsing").reduce((a, s) => a + (s.duration_minutes || 0), 0);
  const browsingMinutes = sessions.filter(s => s.session_type === "browsing").reduce((a, s) => a + (s.duration_minutes || 0), 0);
  const totalCards = sessions.filter(s => s.session_type !== "browsing").reduce((a, s) => a + (s.cards_reviewed || 0), 0);
  ...
  const avgScore = allScoredSessions.length > 0
    ? Math.round(allScoredSessions.reduce((a, s) => a + (s.quiz_score || 0), 0) / allScoredSessions.length)
    : 0;
  ...
  const last7 = Array.from({ length: 7 }).map((_, i) => {
    ...
    return {
      day: format(day, "EEE"),
      minutes: daySessions.reduce((a, s) => a + (s.duration_minutes || 0), 0),
      studyMinutes: daySessions.filter(s => s.session_type !== "browsing").reduce((a, s) => a + (s.duration_minutes || 0), 0),
      browsingMinutes: daySessions.filter(s => s.session_type === "browsing").reduce((a, s) => a + (s.duration_minutes || 0), 0),
    };
  });

  const today = new Date().toISOString().slice(0, 10);
  const todaySessions = sessions.filter(s => s.created_date?.startsWith(today));
  const todayMinutes = todaySessions.reduce((a, s) => a + (s.duration_minutes || 0), 0);
  const todayCards = todaySessions.filter(s => s.session_type !== "browsing").reduce((a, s) => a + (s.cards_reviewed || 0), 0);
