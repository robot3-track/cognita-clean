...
export default function LynxApiPanel({ cardStyle, mutedStyle }) {
  ...
  const runBigPickleTest = async () => {
    ...
    try {
      // Must proxy through Base44 — direct fetch blocked by CORS
    ...
    <div className="space-y-5">
      <div className="rounded-2xl p-6" style={cardStyle}>
        ...
        <p className="text-sm mb-4" style={mutedStyle}>
          Multi-tier fallback: <strong>Lynx → Big Pickle → Gemini → Cohere → Claude</strong>.
        </p>
        ...
        <div className="rounded-xl p-4 mb-4" style={{ background: "rgba(251,191,36,0.07)", border: "1px solid rgba(251,191,36,0.25)" }}>
          ...
          <ul className="text-xs space-y-1" style={mutedStyle}>
            ...
            <li>2️⃣ <strong>Big Pickle API</strong> — 2nd fallback · free beta · 200k context window</li>
            <li>3️⃣ <strong>Gemini API</strong> — fallback for text, JSON, vision & internet search</li>
            <li>4️⃣ <strong>Cohere API</strong> — fallback for text & JSON (command-r-plus)</li>
            <li>5️⃣ <strong>Claude API</strong> — fallback via Base44 InvokeLLM (claude_sonnet_4_6)</li>
            <li>6️⃣ <strong>Base44</strong> — final fallback (internet/vision use this after Gemini)</li>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            ...
            { label: "🥒 Big Pickle API", color: "text-green-400", lines: [`URL: opencode.ai/zen/v1 (via proxy)`, `Model: ${BIG_PICKLE_MODEL}`, `Key: ${BIG_PICKLE_API_KEY.slice(0, 20)}…`, "Proxied via Base44 (CORS bypass)"] },
            ...
            { label: "🟢 Cohere API", color: "text-teal-400", lines: [`URL: api.cohere.ai/v1/chat`, `Model: ${COHERE_MODEL}`, `Key: ${(COHERE_API_KEY || "").slice(0, 20)}…`, "Use: 4th fallback"] },
            { label: "🟠 Claude API", color: "text-orange-400", lines: [`URL: Base44 InvokeLLM`, `Model: ${CLAUDE_MODEL}`, `Key: ${(CLAUDE_API_KEY || "").slice(0, 20)}…`, "Use: 5th fallback"] },
      ...
      <div className="rounded-2xl p-6" style={cardStyle}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            ...
            { label: "🥒 Big Pickle", fn: runBigPickleTest, loading: bigPickleTesting, result: bigPickleResult, error: bigPickleError, bg: "bg-green-600 hover:bg-green-500 text-white", color: "text-green-400" },
